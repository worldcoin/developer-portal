...
export const UserInfo = (props: UserInfoProps) => {
  ...
  const userQueryRes = useFetchUserQuery({
    ...
    context: { headers: { team_id: 'team_b0b7af3f49ea4b6106332f7b6dd5b708___' } },
    skip: !userId
  })
  ...
  return (
    ...
    >
      <div className="leading-[0]">
        {!userQueryRes.data?.user ? <Skeleton  width={72} height={72} circle={true} inline={true} /> : <Icon name={props.name ?? userQueryRes.data?.user?.name} />}
      </div>

      <div className="grid gap-y-2">
        <Typography variant={TYPOGRAPHY.H6}>
          {!userQueryRes.data?.user ? <Skeleton width={200} /> : props.name ?? userQueryRes.data.user.name}
        </Typography>
