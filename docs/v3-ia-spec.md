...
- **Relationship to `world-id-4-0` (historical — no longer a precondition):** v3 originally rode on the WID-4.0 rollout flag (`portal-v3` ⊆ `world-id-4-0`). But **#1974 removed the WID-4.0 rollout flag** — 4.0 is always available now — so the subset gate is gone: `portal-v3` is a **standalone** SSM allowlist (`isPortalV3EnabledServer` no longer checks 4.0). Since every team is on 4.0, legacy Incognito actions stay read-only as before (gated by `isLegacyActionsEditableForTeam`, which survived the flag removal).
