...
Deploy both parts of this rollout before running `reserve`: the worklist/commands and the companion Portal/MCP activation support. Keep setup paused until both are deployed and all reservation requests are resolved.

## Prepare

1. Deploy the migration, Hasura metadata, setup-pause guards, and companion managed-activation change.
