...
import { ActionStore, useActionStore } from "stores/action-store";
import { AppStore, useAppStore } from "stores/app-store";
import { Action } from "./Action";

const appParams = ({ currentApp }: AppStore) => ({
  currentApp,
});

const actionParams = ({ actions, fetchActions }: ActionStore) => ({
  actions,
  fetchActions,
});
...
export function Actions(): JSX.Element | null {
  ...
  const { currentApp } = useAppStore(appParams);
  const { actions, fetchActions } = useActionStore(actionParams);

  useEffect(() => {
    fetchActions();
    console.log("actions:", actions); // DEBUG
  }, []);

  const filterActions = () => {
    const filtered = actions.filter(
      (action) => action.app_id === currentApp.id && action.action !== ""
    );
    return filtered;
  };
