...
describe("parseTotalsTable", () => {
  ...
  it("accepts legacy totals metric names during the schema rollout", () => {
    const legacyCsv = [
    ...
    expect(parseTotalsTable(legacyCsv).records.get(appIdA)).toEqual({
      appId: appIdA,
      n_users_started_at_least_one_selfie_check_flow: 10,
      n_users_shared_at_least_one_proof: 8,
      n_selfie_check_started_sessions: null,
      n_face_capture_started_sessions: 9,
      n_face_capture_completed_sessions: 8,
      n_proof_shared_sessions: 3,
      p_selfie_check_to_face_capture_started_completion: null,
      p_face_capture_started_to_completed_completion: 0.75,
...
describe("parseDailyTable", () => {
  ...
  it("accepts legacy daily metric names during the schema rollout", () => {
    const legacyHeader =
    ...
    const result = parseDailyTable(
      [legacyHeader, `${appIdA},2026-08-26,iOS,10,8,20,0.75`].join("\n"),
    );

    expect(result.records.get(appIdA)?.[0]).toEqual(
      expect.objectContaining({
  ...
  it("rejects ambiguous legacy and current names for the same metric", () => {
    expect(() =>
      parseDailyTable(`${dailyHeader},N_PROOF_USERS\n${dailyRow()},8`),
    ).toThrow("multiple columns for metric n_users_shared_a_proof");
  });
});
// #endregion
...
// #region Current warehouse exports
describe("current selfie-check warehouse exports", () => {
  const repoRoot = path.resolve(__dirname, "../../../../");

  it("parses and keys the new totals CSV by PARTNER_APP_ID", () => {
    const csv = readFileSync(
      path.join(repoRoot, "new_totals_tables.csv"),
      "utf8",
    );
    const table = parseTotalsTable(csv);

    expect(table.headers).toEqual(totalsCsv().split("\n")[0]!.split(","));
    expect(table.records.size).toBeGreaterThan(0);
    for (const [appId, row] of table.records) {
  ...
  it("parses and groups the new daily CSV by PARTNER_APP_ID", () => {
    const csv = readFileSync(
      path.join(repoRoot, "data_20260903_220231.csv"),
      "utf8",
    );
    const table = parseDailyTable(csv);

    expect(table.headers).toEqual(dailyHeader.split(","));
    expect(table.records.size).toBeGreaterThan(0);
    for (const [appId, rows] of table.records) {
      expect(rows.length).toBeGreaterThan(0);
      expect(rows.every((row) => row.appId === appId)).toBe(true);
      expect(rows[0]).toHaveProperty("n_users_shared_a_proof");
      expect(rows[0]).toHaveProperty("p_face_capture_completion");
    }
  });
});
// #endregion
