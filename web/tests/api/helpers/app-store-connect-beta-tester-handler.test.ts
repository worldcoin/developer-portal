...
beforeAll(async () => {
  ...
  encodedPrivateKey = Buffer.from(await exportPKCS8(privateKey)).toString(
    "base64",
  );
});
...
beforeEach(() => {
  ...
  process.env.APP_STORE_CONNECT_API_KEY_CONTENT = encodedPrivateKey;
...
describe("App Store Connect configuration", () => {
  it("decodes the base64-encoded PKCS#8 private key", () => {
    const privateKey = getAppStoreConnectConfig().privateKey;

    expect(privateKey).toMatch(/^-----BEGIN PRIVATE KEY-----/);
    expect(privateKey).toMatch(/-----END PRIVATE KEY-----$/);
  });

  it("rejects malformed base64 private-key content", () => {
    process.env.APP_STORE_CONNECT_API_KEY_CONTENT = "not-valid-base64";

    expect(() => getAppStoreConnectConfig()).toThrow(
      "APP_STORE_CONNECT_API_KEY_CONTENT must be a base64-encoded PKCS#8 PEM private key",
    );
  });

  it("rejects base64 content that does not decode to a PKCS#8 PEM key", () => {
