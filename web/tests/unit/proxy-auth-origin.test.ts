...
describe("proxy [protected route role restrictions]", () => {
  ...
  it("allows a member to open the consolidated team settings page", async () => {
    // Regression guard for a deliberate permission change: team settings used
    // to be owner-only, but is now open to Owner/Admin/Member for everyone —
    // the page itself scopes what each role can see/do.
