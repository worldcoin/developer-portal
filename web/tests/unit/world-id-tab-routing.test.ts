...
describe("actions layout [routes to v4 when there is nothing legacy]", () => {
  it("redirects to the World ID 4.0 surface when the app has no legacy actions", async () => {
    withAppEnv({ actions: [] });
  ...
  it("does not redirect when the app still has legacy actions", async () => {
    withAppEnv({ actions: [{ id: "action_legacy" }] });
