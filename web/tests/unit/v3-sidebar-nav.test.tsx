...
describe("SidebarNav", () => {
  it("marks the active route with aria-current and leaves others unmarked", () => {
    ...
    render(<SidebarNav />);
    expect(screen.getByText("Configuration").closest("a")).toHaveAttribute("aria-current", "page");
    expect(screen.getByText("World ID").closest("a")).not.toHaveAttribute("aria-current");
  });
  ...
  it("without a selected app, app-scope items point to the apps list and team-scope items link normally", () => {
    ...
    render(<SidebarNav />);
    expect(screen.getByText("Dashboard").closest("a")).toHaveAttribute("href", "/teams/team_1/apps");
    expect(screen.getByText("API Keys").closest("a")).toHaveAttribute("href", "/teams/team_1/api-keys");
  });
});
