...
// LivePreview only needs BasicInformation's draft atom. Mocking the component
// avoids loading its Auth0-backed editing surface in this cache integration
// test.
jest.mock(
  "@/scenes/PortalV3/Teams/TeamId/Apps/AppId/Configuration/BasicInformation",
  () => {
    const { atom } = jest.requireActual("jotai");
    return { basicInfoDraftAtom: atom({}) };
  },
);
...
describe("image upload [success path]", () => {
  ...
  it("appends a second showcase image without refetching the first preview", async () => {
    const firstUrl =
      "https://assets.test/unverified/app/showcase_img_1.png?signature=first";
    const secondUrl =
      "https://assets.test/unverified/app/showcase_img_2.png?signature=second";
    const captured: unknown[] = [];
    const s3Gate = deferred();
    s3Gate.resolve();
    stubS3(s3Gate);
...
// #region locale-aware live preview
const LocalizedLivePreviewHarness = () => {
  const form = useForm({
    defaultValues: {
      localisations: [
        {
...
describe("localized live preview", () => {
  it("renders the selected locale's cache entry and reacts to its local cache update", async () => {
    const client = new ApolloClient({
      link: new MockLink([]),
      cache: new InMemoryCache(),
    });
    ...
    const initialUrl =
      "https://assets.test/unverified/app/es/showcase_img_1.png?signature=old";
    const nextUrl =
