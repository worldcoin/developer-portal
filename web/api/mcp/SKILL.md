...
| `set_world_id_staging_verification` | Open (or close) a 24h window in which `/api/v4/verify` accepts staging / sandbox proofs, and issue the token those calls must send as `x-staging-verification-token` | `app_id`, `enabled`                                                                                                                                      |
