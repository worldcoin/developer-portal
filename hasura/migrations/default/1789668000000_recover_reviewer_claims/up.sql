...
DECLARE
    ...
    recovering_existing_claim boolean := false;
BEGIN
    SELECT submission.*
    INTO candidate
    ...
    IF NOT FOUND OR candidate.status NOT IN ('pending', 'in_review') THEN
        RETURN;
    END IF;

    recovering_existing_claim :=
        candidate.claim_token IS NOT NULL
        AND candidate.claim_expires_at > now();

    -- A valid lease remains exclusive, but its authenticated owner may rotate
    -- the opaque token after losing tab storage or reopening the review.
    IF recovering_existing_claim
       AND candidate.claimed_by_subject IS DISTINCT FROM p_actor_subject THEN
        RETURN;
    END IF;
    ...
    SET status = 'in_review',
        ...
        claimed_at = CASE
            WHEN recovering_existing_claim THEN candidate.claimed_at
            ELSE now()
        END,
    ...
    IF candidate.claim_token IS NOT NULL
       AND NOT recovering_existing_claim THEN
    ...
    VALUES (
        ...
        jsonb_build_object(
            ...
            'previous_claim_expires_at', CASE
                WHEN recovering_existing_claim THEN candidate.claim_expires_at
                ELSE NULL
            END
        )
    );
...
COMMENT ON FUNCTION public.reviewer_claim_app_review_submission(uuid, integer, text, text) IS
'Claims an available review or rotates an unexpired claim token for the same authenticated reviewer.';
