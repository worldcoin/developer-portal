...
BEGIN
  ...
  IF _repaired > 0 OR _repaired_actions > 0 OR _repaired_apps > 0 THEN
    RAISE WARNING 'verification reconciliation repaired: stranded_deltas=% missing_action_totals=% missing_app_totals=% (batch through %)',
      _repaired, _repaired_actions, _repaired_apps, _last_id;
  END IF;
  ...
  RETURN QUERY SELECT 'reconciliation'::text,
    ...
    format('stranded=%s,repaired_actions=%s,repaired_apps=%s,action_mismatch=%s,app_mismatch=%s,impossible=%s,last_action_id=%s',
           _repaired, _repaired_actions, _repaired_apps,
