...
import { createTestTeam, createTestUser, deleteTestTeam, deleteTestUser } from '../../helpers/hasura';
  ...
  describe('POST /api/create-team', () => {
    ...
    it('Create Team Successfully For New User', async () => {
      ...
      try {
        ...
        expect(response.data).toEqual(
          expect.objectContaining({
            returnTo: expect.stringMatching(/^\/teams\/team_[a-f0-9]{32}\/apps\/$/)
          })
        );
      } catch (error: any) {
        ...
        if (error.response?.status === 500) {
          ...
          if (detail.includes('Failed to send acceptance') || 
