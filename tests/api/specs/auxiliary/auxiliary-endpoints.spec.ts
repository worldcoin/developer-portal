...
describe('Auxiliary API Endpoints', () => {
  ...
  describe('POST /api/_evaluate-app-notification-permissions', () => {
    it('Evaluate Notification Permissions With Authorization', async () => {
      const response = await axios.post(
        `${INTERNAL_API_URL}/api/_evaluate-app-notification-permissions`,
        {},
        {
          headers: {
            'Authorization': `Bearer ${token}`,
