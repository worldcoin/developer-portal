...
describe('Hasura API - Get Unverified Images', () => {
  describe('POST /api/hasura/get-unverified-images', () => {
    ...
    beforeAll(async () => {
      ...
      testUserId = await createTestUser(`getunverified_${Date.now()}@example.com`, testTeamId!);
      ...
      // Create test app
      testAppId = await createTestApp('Test App for Get Unverified Images', testTeamId!);
